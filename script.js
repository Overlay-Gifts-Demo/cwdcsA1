...
async function getLatestVideoId() {
  try {
    // UPDATED: Removed the localStorage check to force a fresh fetch every time
    ...
    if (data.files && data.files.length > 0) {
      return data.files[0].id; // Return the ID directly without saving to memory
    }
    return null;
  } catch (error) {
